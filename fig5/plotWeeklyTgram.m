...
MARK_SCHOOL_HOLIDAYS = 1; %mark school holidays (1=yes)
MARK_PUBLIC_HOLIDAYS = 1; %mark federal/public holidays (1=yes)
...
CLUST_file = load('datafiles/SJLtable_clustAssign_2018-04-20_11.25.24.mat');
...
% calendar data
CALFILE = ['datafiles/CountyTable_SchoolCalendar.mat'];
calTab = load(CALFILE);
...
set(gca,'xlim',[0 53],'xtick',1:4:52,... % 'ylim',[min(tr) max(tr)] + [-1 0.75]
...
function [hIm,hAx] = heatMapTgram(tgram,titStr,troughs,varargin)
    myParser = inputParser;
    addParameter(myParser,'ylim',[1 52],@isnumeric);
    addParameter(myParser,'titStr','',@ischar);
