...
%which aalysis would you like to do? export figures? (1=yes)
...
TOEXPBARS = 1;  %export Fig. 2E, Fig. S2A? 
TOVIF = 0; %calculate VIF
...
if TOVIF == 1
    ...
    for j = 1:NumPredictors
        ...
        %% linear regression
        mdl = myLinModel(OthersTable,PredictorJ,'linear','toCV',0,'truthplot',0);
        JVIF = 1 / (1 - mdl.Rsquared.Adjusted);
        VIFList(j) = JVIF;
    end
    ...
    handVIFList = zeros(1,NumPredictors);
    for j = 1:NumPredictors
        %% make table excluding column j + 1
        OthersTable = handVIFTable(:,1:NumPredictors);
        PredictorJ = OthersTable(:,j);
        OthersTable(:,j) = [];
...
for i=1:numVars
    [corrList(i), pList(i)] = ...
        corr(yTab.(responseVar),xTab{:,i},'rows','complete');
end
